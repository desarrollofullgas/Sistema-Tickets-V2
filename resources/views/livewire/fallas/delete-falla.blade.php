<div>
    <button wire:click="ConfirmDelete({{$fallaID}})" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modal" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span>¿Está seguro de eliminar la falla <i>"{{$name}}"</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="deleteFalla({{$fallaID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
