<div>
    <button wire:click="ConfirmDelete({{$zonaID}})" wire:loading.attr="disabled" class="tooltip">
    ...
    <x-dialog-modal wire:model="modalDelete" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span class="dark:text-white">¿Está seguro de eliminar la zona <i>"{{$zName}}"</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="DeleteZona({{$zonaID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
