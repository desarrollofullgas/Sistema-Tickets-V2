<div>
    <button class="absolute top-0 right-0" wire:click="ConfirmDelete({{$manID}})" wire:loading.attr="disabled" title="Eliminar">
    ...
    <x-dialog-modal wire:model="modalDelete" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span class="dark:text-white">¿Está seguro de eliminar el manual <i>"{{$mName}}"</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="DeleteManual({{$manID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
