<div>
    <button wire:click="ConfirmDelete({{$areaID}})" wire:loading.attr="disabled" class="tooltip">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-400 hover:text-red-500">
          ...
          <span class="tooltiptext">Eliminar área</span>
    </button>
    <x-dialog-modal wire:model="modal" class="flex items-center">
        ...
        <x-slot name="content" class="relative">
            <div>
                ...
                <span>¿Está seguro de eliminar a <i>"{{$areaName}}"</i> ?</span>
            </div>
        </x-slot>
        <x-slot name="footer" >
            <div class="w-full flex justify-center gap-3">
                <x-danger-button class="mr-2" wire:click="deleteArea({{$areaID}})" wire:loading.attr="disabled">
                    Aceptar
                </x-danger-button>
