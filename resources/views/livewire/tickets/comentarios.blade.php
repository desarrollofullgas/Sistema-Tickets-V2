        ...
        <x-slot name="content">
			...
			@if (Auth::id() != $ticketOwner || $ticketOwner == $tck->user_id)
                    <div class="flex flex-wrap gap-3 justify-evenly items-center">
                        <div>
                            <x-label value="{{ __('Estado del ticket') }}" for="status" />
                            <select wire:model="status" name="status" id="status"
                                class=" border-gray-300 rounded-md dark:bg-slate-800 dark:border-gray-700 dark:text-gray-400">
                ...
                <div class="w-full">
                    ...
                    <textarea wire:model="mensaje"
                        class="resize-none w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm dark:bg-slate-800 dark:border-gray-700 dark:text-gray-400 {{ $errors->has('mensaje') ? 'is-invalid' : '' }} resize-none"
                ...
                <div class="mb-3 col-12 w-full" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true"
                    ...
                    <input type="file" wire:model="evidencias"
                        class="flex flex-wrap file:text-sm file:font-semibold file:bg-blue-300 file:text-blue-700 hover:file:bg-blue-100 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 dark:text-gray-300"
