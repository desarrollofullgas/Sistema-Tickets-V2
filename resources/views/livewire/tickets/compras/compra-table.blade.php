<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-full md:w-auto" placeholder="Buscar requisiciones..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                ...
                <x-slot name="body">
                    @forelse($compras as $compra)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            ...
                            <x-cell class="font-bold">#{{ $compra->id }} </x-cell>
                            <x-cell class="font-bold">#{{ $compra->ticket_id }}</x-cell>
                            <x-cell>{{ $compra->ticket->agente->name }}</x-cell>
                            <x-cell>{{ $compra->ticket->cliente->name }}</x-cell>
                            <x-cell>
                                <div
                                    class="rounded bg-{{ $compra->status_color }}-200 py-1 px-3 text-center text-xs text-{{ $compra->status_color }}-500 font-bold">
                                    {{ $compra->status }}
                                </div>
                            </x-cell>
                            ...
                            <x-cell>
                                <div class="flex gap-2 justify-center items-center">
                                    ...
                                    @if ($compra->status != 'Completado' && $compra->status != 'Rechazado')
                                        @if (
                                            (Auth::user()->permiso_id == 1 && $compra->status == 'Solicitado') ||
                                                (Auth::user()->permiso_id == 4 && $compra->status == 'Solicitado') || (Auth::user()->permiso_id == 4 || $compra->status == 'Aprobado' || $compra->status == 'Enviado a compras') )
