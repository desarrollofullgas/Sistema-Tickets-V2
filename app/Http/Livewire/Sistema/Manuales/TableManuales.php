    ...
    public function getManualsQueryProperty()
    {
        ...
        return Manual::search($this->search)
            ->when($user->permiso_id == 1 || $user->permiso_id == 8, function ($query) {
                // Si el usuario es un administrador/Auditoria, no aplicamos restricciones
                return $query;
            }, function ($query) {
                // Si el usuario es distinto a admin o auditoria, filtramos por sus permiso asignados
