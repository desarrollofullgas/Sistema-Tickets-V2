...
class Ranking extends Component
{
    ...
    public function mount(){
        ...
        $usuarios=User::whereHas('tickets',function(Builder $query) use($dia){
            $query->where('status','Cerrado')->whereBetween('created_at',[$dia->startOfMonth()->toDateTimeString(),$dia->endOfMonth()->toDateTimeString()]);
        })->get();
    ...
    public function calificaciones($users,$dia){
        ...
        foreach($users as $user){
            $pos=0;
            $neg=0;     
            $tcks=$user->tickets->whereBetween('created_at',[$dia->startOfMonth()->toDateTimeString(),$dia->endOfMonth()->toDateTimeString()])->where('cerrado','!=',null);
            ...
            array_push($arrUsers,[
                ...
                'neg'=>number_format($neg,2),
                'cal'=>number_format(($pos*100/($pos+$neg)),2),
                'total'=>number_format(($pos+$neg),2)
            ]);
        }
